...
def run_exporter(exporter_path: Path, pokerogue_root: Path, dry_run=False):
    cmd = [sys.executable, str(exporter_path), "--pokerogue-root", str(pokerogue_root)]
    run_command(cmd, dry_run=dry_run)


def main():
    repo_root = Path(__file__).resolve().parents[1]
    default_pokerogue_root = repo_root / "dependency" / "pokerogue"
