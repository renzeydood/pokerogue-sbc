extends BattlePhase
class_name CapturePostEncounterPhase
...
func _init(battle, context: Dictionary, active_turn_token: int) -> void:
	phase_name = "CapturePostEncounterPhase"
	_battle = battle
	_context = context
...
func _on_start() -> void:
	...
	if bool(capture_state.get("cancelled", false)):
		if _battle.has_method("_log_capture_checkpoint"):
			_battle._log_capture_checkpoint("phase.post.skipped_cancelled")
		var _completed = complete()
		return

	if _battle.has_method("_log_capture_checkpoint"):
		_battle._log_capture_checkpoint("phase.post.start", {
			"capture_success": bool(capture_state.get("capture_success", false)),
		})

	var post_result = _battle._run_capture_post_encounter_phase_state(capture_state, _active_turn_token)
	if post_result is GDScriptFunctionState:
		post_result = yield(post_result, "completed")
	if typeof(post_result) == TYPE_DICTIONARY:
		capture_state = post_result
	_context["capture_state"] = capture_state

	if _battle.has_method("_log_capture_checkpoint"):
		_battle._log_capture_checkpoint("phase.post.complete", {
